...
## Determining fast_growing 
#  sales_change: checking the percentage change in sales_mil from this year to the next year
#  a company is fast_growing when the sales_change percentage is more than 30%
#  I determined 30% as fast growth for a year-on-year change in sales

data <- data %>%
  group_by(comp_id) %>%
  mutate(sales_change = (lead(sales_mil, 1) - sales_mil ) / sales_mil ) %>%
  ungroup()

data  <- data %>%
  mutate(fast_growing = sales_change > 0.3 & !is.na(sales_change) %>%
           as.numeric(.))
...
# We are looking at the year 2012 and whether a company is still alive
# look at cross section
data <- data %>%
  filter((year == 2012) & (status_alive == 1)) %>%
  # look at firms below 10m euro revenues and above 10,000 euros, I think that 1000 euros is too small for an annual sales figure
  filter(!(sales_mil > 10)) %>%
...
data <- data %>%
  mutate(fast_growing_f = factor(fast_growing, levels = c(FALSE,TRUE)) %>%
           recode(., `FALSE` = 'no_fast_growing', `TRUE` = "fast_growing"))
...
ggplot(data = data, aes(x=sales_mil_log, y=as.numeric(fast_growing))) +
  ...
  labs(x = "sales_mil_log",y = "fast_growing") +
  theme_bg()


ols_s <- lm(fast_growing~sales_mil_log+sales_mil_log_sq,
                data = data)
summary(ols_s)
...
d1sale_1<-ggplot(data = data, aes(x=d1_sales_mil_log, y=as.numeric(fast_growing))) +
