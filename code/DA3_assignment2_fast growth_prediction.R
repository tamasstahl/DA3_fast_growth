...
rf_for_graph <-
  rpart(
    formula = fast_growing_f ~ sales_mil + profit_loss_year+ foreign_management + age + curr_liab_bs + profit_loss_year_pl + fixed_assets_bs,
    data = data_for_graph,
...
fp <- sum(data_holdout$rf_f_prediction_class == "fast_growing" & data_holdout$fast_growing_f == "no_fast_growing")
fn <- sum(data_holdout$rf_f_prediction_class == "no_fast_growing" & data_holdout$fast_growing_f == "fast_growing")
